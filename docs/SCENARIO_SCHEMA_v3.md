# Scenario Schema v3 (Open-Ended DM Tabletop)

v3 builds on v2's open-ended, LLM-run model and makes three changes:
...
```jsonc
{
  ...
  // DM is the LLM), so there are no moderator-only notes displayed to players.
  "intro": {
